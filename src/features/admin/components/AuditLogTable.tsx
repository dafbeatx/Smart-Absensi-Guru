              ...
              <tr key={log.id} className="hover:bg-slate-50/50">
                ...
                <td className="p-3">
                  <span className={`inline-block px-2 py-0.5 rounded-md text-[10px] font-bold ${
                    log.actor_role === 'ADMIN' || log.actor_role === 'OPERATOR' ? 'bg-blue-100 text-blue-800' : log.actor_role === 'KEPSEK' ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'
                  }`}>
                    {log.actor_role === 'ADMIN' || log.actor_role === 'OPERATOR' ? 'ADMIN WEBSITE' : log.actor_role}
                  </span>
