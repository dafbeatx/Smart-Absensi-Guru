...
export const BurnoutEarlyWarningWidget: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-3xl p-5 border border-[#D4D4CE]/40 shadow-card animate-pulse space-y-4">
  ...
  return (
    <div className="bg-white rounded-3xl p-5 sm:p-6 border border-[#D4D4CE]/40 shadow-card space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5 font-medium">
              Monitoring tingkat energi, kelelahan, dan kesejahteraan psikologis guru.
            </p>
          </div>
        </div>

        {/* Privacy Guarantee Badge */}
        <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-600 font-bold shrink-0">
          <span>🔒</span>
      ...
      {total === 0 ? (
        /* Bright Empty State when No Mood Logged Yet */
        <div className="bg-linear-to-r from-emerald-50/80 via-teal-50/50 to-slate-50 p-6 rounded-2xl border border-dashed border-emerald-200 text-center space-y-2">
          ...
          <div>
            <h4 className="font-extrabold text-[#023246] text-sm">
              Belum Ada Respon Mood Guru Hari Ini
            </h4>
            <p className="text-xs text-slate-500 max-w-md mx-auto mt-1 font-medium leading-relaxed">
              Grafik statistik dan rekomendasi burnout akan otomatis muncul secara realtime begitu dewan guru mengisi <b>Mood Check-in</b> saat presensi.
            </p>
          </div>
          <div className="pt-1">
            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-white border border-slate-200 text-[11px] font-bold text-slate-600 shadow-2xs">
              <span>📊 Status:</span>
              <span className="text-emerald-700">0 Respon Tercatat</span>
            </span>
          </div>
        </div>
      ) : (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="md:col-span-2 bg-slate-50/90 p-4 rounded-2xl border border-slate-200/80 space-y-2.5">
              <span className="text-[11px] font-extrabold text-slate-500 uppercase tracking-wider block">
                Distribusi Suasana Hati / Mood Guru
              </span>
